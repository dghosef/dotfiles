...
(global-set-key (kbd "M-o") 'last-window)
...
(defun insert_brackets ()
  ...
  (if (not (looking-back " "))
	  (insert " "))
  ;; Record if we're in a struct
  (evil-first-non-blank)
  (let ((struct (string= (thing-at-point 'word 'no-properties) "struct"))
		(class (string= (thing-at-point 'word 'no-properties) "class")))
	(end-of-visual-line)
	(insert "{")
